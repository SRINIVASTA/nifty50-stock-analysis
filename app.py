...
ax1 = sns.barplot(x='Ticker', y='Current Price', data=df, color='skyblue', label='Current Price', width=0.6)
ax1.set_ylim(0, max(df['Current Price'].max(), df['Book Value'].max()) * 1.1)

for i, txt in enumerate(df['Book Value']):
    ax1.annotate(f"{txt:.2f}", (df.index[i], df['Book Value'].iloc[i]), textcoords="offset points", xytext=(0, 10), ha='center', fontsize=8, rotation=90)

ax1.plot(df.index, df['Book Value'], color='darkgreen', linewidth=2, marker='o', linestyle='-', label='Book Value')

ax2 = ax1.twinx()
ax2.plot(df.index, df['P/B Ratio'], color='red', linewidth=2, linestyle='--', marker='x', label='P/B Ratio')
ax2.set_ylabel('P/B Ratio', color='red', fontsize=12)
ax2.tick_params(axis='y', labelcolor='red', labelsize=10)

xlabels = [item.get_text() for item in ax1.get_xticklabels()]
ax1.set_xticks(ax1.get_xticks())
ax1.set_xticklabels(xlabels, rotation=90, ha='right', fontsize=10)
